<template>
  <div class="main">
    ...
    <el-dialog  :title="title" :visible.sync="dialog" width="50%">
      ...
      <el-form ref="form" :model="arrangement" label-width="80px">
        <el-form-item label="电影id">
          <el-input :disabled="flag !== true" v-model="arrangement.fid"></el-input>
        </el-form-item>
        <el-form-item label="电影名称">
          <el-input :disabled="flag !== true" v-model="arrangement.name"></el-input>
        </el-form-item>
        <el-form-item label="开放座位">
          <el-input-number v-model="arrangement.seat_number" :min="10" :max="100"></el-input-number>
        </el-form-item>
        ...
        <el-form-item style="width: 500px" label="放映时间">
          <el-col :span="11">
            <el-time-picker placeholder="开始时间" value-format="HH:mm:ss" v-model="arrangement.start_time"
              style="width: 100%;"></el-time-picker>
          </el-col>
          <el-col style="text-align: center" class="line" :span="2">-</el-col>
          <el-col :span="11">
            <el-time-picker placeholder="结束时间" value-format="HH:mm:ss" v-model="arrangement.end_time"
              style="width: 100%;"></el-time-picker>
          </el-col>
        </el-form-item>
    ...
    <el-table v-loading="loading"
      ...
      <el-table-column label="开放座位" prop="seat_number">
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$axios
        .get("http://localhost:8080/arrangement/select")
    ...
    methods: {
      ...
      Arrangedel(id) {
        const that = this;
        this.$axios
          .get("http://localhost:8080/arrangement/delete", {
            params: {
              id: id,
            },
          })
      ...
      submitEditArrange() {
        ...
        if (this.flag ==false) {
          this.$axios({
            method: 'post',
            url: 'http://localhost:8080/arrangement/update',
          ...
          if (this.arrangement.name !=='' &&  this.arrangement.seat_number !== '' && this.arrangement.start_time !== '' && this.arrangement.end_time !== '' && this.arrangement.type !== '' && this.arrangement.price !== '' && this.arrangement.date !== '') {
            this.$axios({
              method: 'post',
              url: 'http://localhost:8080/arrangement/add',
              data: this.arrangement
            }
            ).then(function (response) {
              if (response.data.success == true) {
